...
class Fluid():
    ...
    def __init__(self, mu=0.00089, rho=1000):
        ...
        self.nu= self.mu/self.rho#JES MISSING CODE$ # calculate the kinematic viscosity in units of m^2/s
    #endregion
class Node():
    #region constructor
...
class Pipe():
    ...
    def frictionHeadLoss(self):  # calculate headloss through a section of pipe in m of fluid
        ...
        v = self.V()
        hl = (ff * self.length * v**2) / (self.d * 2 * g)#$JES MISSING CODE$ # calculate the head loss in m of water
        return hl
...
class PipeNetwork():
    ...
    def findFlowRates(self):
        ...
        def fn(q):
            ...
            L += self.getLoopHeadLosses() #$JES MISSING CODE$  # call the getLoopHeadLosses function of this class
            return L
    ...
    def getNodeFlowRates(self):
        '''
        Method to calculate the net flow rates at nodes.
        :return: array of net flow rates at nodes
        '''
        node_flow_rates = []
        for node in self.nodes:
            net_flow_rate = sum(pipe.Q for pipe in node.connected_pipes)
            node_flow_rates.append(net_flow_rate)
        return node_flow_rates

    def getLoopHeadLosses(self):
        '''
        Method to calculate the net head losses in loops.
        :return: array of net head losses in loops
        '''
        loop_head_losses = []
        for loop in self.loops:
            net_head_loss = sum(pipe.head_loss for pipe in loop.connected_pipes)
            loop_head_losses.append(net_head_loss)
        return loop_head_losses
...
def main():
    ...
    PN=PipeNetwork(fluid=water)#$JES MISSING CODE$  #
